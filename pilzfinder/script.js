...
function generate() {
  ...
  query({
    ...
    prompt: "Maronen Röhrling*Frauen Täubling*Violetter Rötelritterling*Eichen Milchling*Knollen Röhrling*Flockenstieliger Hexenröhrling*Gewaltigen Steppen Täubling*Rotfußröhrling*Hainbuchen Milchling*Gemeiner Wulstling*Ziegen Milchling*FichtenreizkerBlutmilchender Parasol*Satans Röhrling*Schopftintling*Eichen Täubling*Gemeiner Riesenschirmling*",
    max_tokens: 70,
    ...
    if (sauphemismus.length > nice || sauphemismus.length < 3 || isNaughty(sauphemismus)) {
      console.log("Regenerating, length: " + sauphemismus.length + "\tNaughty: " + isNaughty(sauphemismus));
      generate();
      return;
    }
    ...
    try {
      ...
      history.innerHTML +=
        '<li><a href="whatsapp://send?text=https://sauphemismus.github.io/en%0a%0aOh, ist das ein' +
        last_sauphi +
        '">Oh, ist das ein' +
        last_sauphi +
        "</a></li>";
    ...
    document.getElementById(
      "share"
    ).innerHTML = `<button class="button2" style="border-bottom: 0.2em solid #fff;" onclick="location.href='whatsapp://send?text=https://sauphemismus.github.io/en%0a%0aOh, ist das ein ${sauphemismus}?';" id="whatsapp">
      ${whatsapp_btn}
      <p>Invite</p>  
        </button>`;
